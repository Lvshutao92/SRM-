...
@interface AddSiYouViewController ()<UITextFieldDelegate,UITextViewDelegate,PGDatePickerDelegate,UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate>
{
...
@implementation AddSiYouViewController
...
- (void)viewDidLoad {
    ...
    if ([self.navigationItem.title isEqualToString:@"编辑私有线索"]) {
        text1.text = self.model.leadGetDate;
        text2.text = self.model.leadSource;
        text3.text = self.model.customerShortName;
        text11.text = self.model.followPersonName;
        followUserId = self.model.followUserId;
    ...
    self.tableview = [[UITableView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT-350-10-50, SCREEN_WIDTH, 350)];
    self.tableview.delegate = self;
    self.tableview.dataSource = self;
    [self.tableview registerClass:[UITableViewCell class] forCellReuseIdentifier:@"cell"];
    [window addSubview:self.tableview];
    UIView *vv = [[UIView alloc]initWithFrame:CGRectMake(0,0, SCREEN_WIDTH, 0.1)];
    self.tableview.tableFooterView = vv;
    
    UILabel *hea = [[UILabel alloc]initWithFrame:CGRectMake(0,SCREEN_HEIGHT-451, SCREEN_WIDTH, 40)];
    hea.text = @"请选择";
    ...
    btn.frame = CGRectMake(0, SCREEN_HEIGHT-10-40, SCREEN_WIDTH-0, 40);
...
- (void)clicksave{
    if ([self.navigationItem.title isEqualToString:@"新增私有线索"]) {
        [self addgongyou];
    }else{
        [self editgongyou];
    }
...
- (void)addgongyou{
    ...
    if (text1.text.length >0 && text2.text.length >0 && text3.text.length >0 && followUserId.length >0) {
        ...
        [Manager requestPOSTWithURLStr:[Manager dictionToString:dic string:KURLNSString(@"lead/private/insert?")] finish:^(id  _Nonnull responseObject) {
...
//
        ...
        [Manager requestPOSTWithURLStr:[Manager dictionToString:dic string:KURLNSString(@"lead/private/update?")] finish:^(id  _Nonnull responseObject) {
            NSDictionary *diction = [Manager returndictiondata:responseObject];
//            NSLog(@"------%@",diction);
            if ([[diction objectForKey:@"code"]isEqualToString:@"success"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    }else if ([addressOrother isEqualToString:@"2"]) {
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    if ([textField isEqual:text1]) {
        ...
        str = @"3";
        return NO;
    }else if ([textField isEqual:text11]) {
        [self textFV];
        [self lod7];
        str = @"7";
        return NO;
...
- (void)lod7{
    LRWeakSelf(self);
    [Manager requestPOSTWithURLStr:KURLNSString(@"sys/user/getMineAndChildren")  finish:^(id  _Nonnull responseObject) {
        NSDictionary *diction = [Manager returndictiondata:responseObject];
       // NSLog(@"-----------%@",diction);
        NSMutableArray *arr = (NSMutableArray *)diction;
        if (arr.count == 0) {
...
- (void)setUpView{
    ...
    UILabel *lab5 = [[UILabel alloc]initWithFrame:CGRectMake(0, 250, SCREEN_WIDTH, 40)];
    lab5.font = FONT(20);
    lab5.text = @"   补充信息";
    ...
    UILabel *lab6 = [[UILabel alloc]initWithFrame:CGRectMake(10, 300, 70, 40)];
    lab6.text = @"客户全称";
    [scrollview addSubview:lab6];
    
    UILabel *lab7 = [[UILabel alloc]initWithFrame:CGRectMake(10, 350, 70, 40)];
    lab7.text = @"客户规模";
    [scrollview addSubview:lab7];
    
    UILabel *lab8 = [[UILabel alloc]initWithFrame:CGRectMake(10, 400, 70, 40)];
    lab8.text = @"客户行业";
    [scrollview addSubview:lab8];
    
    UILabel *lab9 = [[UILabel alloc]initWithFrame:CGRectMake(10, 450, 70, 40)];
    lab9.text = @"省";
    [scrollview addSubview:lab9];
    
    UILabel *lab10 = [[UILabel alloc]initWithFrame:CGRectMake(10, 500, 70, 40)];
    lab10.text = @"市";
    [scrollview addSubview:lab10];
    
    UILabel *lab11 = [[UILabel alloc]initWithFrame:CGRectMake(10, 550, 70, 40)];
    lab11.text = @"区";
    [scrollview addSubview:lab11];
    
    
    UILabel *lab12 = [[UILabel alloc]initWithFrame:CGRectMake(10, 600, 70, 40)];
    lab12.text = @"详细地址";
    [scrollview addSubview:lab12];
    
    UILabel *lab13 = [[UILabel alloc]initWithFrame:CGRectMake(10, 650, 70, 40)];
    lab13.text = @"客户介绍";
...
- (void)setBaseInfo{
    ...
    text11 = [[UITextField alloc]initWithFrame:CGRectMake(90, 200, SCREEN_WIDTH-100, 40)];
    text11.delegate = self;
    text11.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text11];
}

- (void)buchongInfo{
    text4 = [[UITextField alloc]initWithFrame:CGRectMake(90, 300, SCREEN_WIDTH-100, 40)];
    text4.delegate = self;
    text4.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text4];
    
    text5 = [[UITextField alloc]initWithFrame:CGRectMake(90, 350, SCREEN_WIDTH-100, 40)];
    text5.delegate = self;
    text5.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text5];
    
    text6 = [[UITextField alloc]initWithFrame:CGRectMake(90, 400, SCREEN_WIDTH-100, 40)];
    text6.delegate = self;
    text6.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text6];
    
    text7 = [[UITextField alloc]initWithFrame:CGRectMake(90, 450, SCREEN_WIDTH-100, 40)];
    text7.delegate = self;
    text7.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text7];
    
    text8 = [[UITextField alloc]initWithFrame:CGRectMake(90, 500, SCREEN_WIDTH-100, 40)];
    text8.delegate = self;
    text8.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text8];
    
    text9 = [[UITextField alloc]initWithFrame:CGRectMake(90, 550, SCREEN_WIDTH-100, 40)];
    text9.delegate = self;
    text9.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text9];
    
    text10 = [[UITextField alloc]initWithFrame:CGRectMake(90, 600, SCREEN_WIDTH-100, 40)];
    text10.delegate = self;
    text10.borderStyle = UITextBorderStyleRoundedRect;
    [scrollview addSubview:text10];
    
    
    textview = [[UITextView alloc]initWithFrame:CGRectMake(90, 650,  SCREEN_WIDTH-100, 120)];
